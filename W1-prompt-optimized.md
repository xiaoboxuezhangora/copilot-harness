...
- 项目零基础，无现成 CI / Skill / MCP；GitLab 已确定为单仓：`http://10.100.77.238/b.w_neu/copilot-harness`
...
在 GitLab `copilot-harness` 仓库下按 monorepo 管理 3 个顶层目录，并补齐根级 README + `.gitignore`（Node）+ LICENSE 或许可说明 + CODEOWNERS：

- `orchestrator/`（编排服务，TS 主体）
- `skills/`（`.github/skills/*/SKILL.md` 集合）
- `playbooks/`（`.github/playbooks/*.md`，Phase 3 才大量写入，先建目录占位）

同时新增 ADR 记录：W1 阶段采用一个 GitLab 仓库管理三条工作流，后续按 Skill 发布/权限隔离需要再拆仓。
...
根级 `.gitlab-ci.yml` 三阶段：`lint` → `typecheck` → `test`
...
- [ ] 根级 `.gitlab-ci.yml` 存在且能从仓库根目录调度 `orchestrator` 质量门
