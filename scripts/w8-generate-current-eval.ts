...
const ROOT_DIR =
  ...
  (process.cwd().endsWith('/orchestrator') ? resolve(process.cwd(), '..') : process.cwd());
...
const DEFAULT_PROFILE_PATH = join(ROOT_DIR, 'orchestrator', 'eval', 'w8-acceptance-profile.json');
...
async function main(): Promise<void> {
  const mappingPath = process.env.W8_CURRENT_EVAL_MAPPING_PATH ?? DEFAULT_MAPPING_PATH;
  const profilePath = process.env.W8_CURRENT_EVAL_PROFILE_PATH ?? DEFAULT_PROFILE_PATH;
  const outputPath = process.env.W8_CURRENT_EVAL_OUTPUT_PATH ?? DEFAULT_OUTPUT_PATH;
  ...
  const confirmed = mapping.issue_mappings
    ...
    .slice(0, profile.required_sample_count);

  if (confirmed.length !== profile.required_sample_count) {
    throw new Error(
      `Expected ${profile.required_sample_count} confirmed mapping samples, found ${confirmed.length}`
    );
  }
    ...
    `${JSON.stringify(
      {
        ...
        projects: [...new Set(samples.map((sample) => sample.ground_truth_repo))],
        refs: [
          ...new Set(
            samples.flatMap((sample) =>
              sample.ground_truth_modules.filter((module) => module.startsWith('ref:'))
            )
          ),
        ],
      },
...
async function buildSample(input: {
  ...
  const evidenceRefs = dedupe([...(candidate.evidence_refs ?? []), await fileEvidence]);
  ...
  return {
    ...
    predicted: {
      ...
      plan: {
        summary: `基于已确认映射处理 ${input.issue.jira_key}。`,
        steps: [
        ...
        files: ['README.md'],
        risks: ['当前为 Jira 样本不足时的 W8 临时收尾数据集，后续需扩展到 50/20 最终标准。'],
...
async function readBranchEvidenceSourceRef(input: {
  ...
  if (typeof sourceRef !== 'string') {
    throw new Error(`Could not read README.md source_ref for ${input.project}@${input.ref}`);
  }
  return sourceRef;
}
...
main().catch((error: unknown) => {
  const message =
    error instanceof Error ? error.message : 'Failed to generate W8 current eval dataset';
